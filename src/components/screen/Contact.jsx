    ...
    <div>
      <div className="text-center h-[300px] contactBanner mt-12 ">
        <div className=" bg-primary-700/80 w-full h-full flex items-center justify-center text-white">
          <div>
            <h1 className="text-[20px] md:text-[25px] lg:text-[32px] font-bold">Feel free to contact us</h1>
            <p className="text-[12px] md:text-[16px] max-w-[400px] mx-auto mt-[8px]">A wonderful serenity has taken possession of my entire soul, like these sweet mornings.</p>
          </div>
        </div>
      </div>
      <div className="flex flex-col lg:flex-row items-center justify-between gap-10 container mt-[64px]">
        <ContactInfo title="Email" info="yourmail@mail.com" linkType="mailto:" />
        <ContactInfo title="Phone" info={["+123 1234 1234", "+123 1234 5451"]} linkType="tel:" />
        <ContactInfo title="Address" info="3612 University Hill Road Sadorus, IL 61872" />
      </div>
      ...
      <div className="flex flex-col lg:flex-row items-start gap-20 my-[64px] container">
        ...
        <div className="w-full">
          ...
          <form className="w-full flex flex-col gap-4" action="#" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-[32px]">
              ...
              <input className="border-[1px] border-[#919EAB]/30 focus:outline-primary-600 rounded-[8px] py-[16px] px-[14px] w-full" type="tel" name="phone" id="phone" placeholder="Phone" />
            </div>
            <div className="">
              <textarea
                className="border-[1px] border-[#919EAB]/30 focus:outline-primary-600 rounded-[8px] py-[16px] px-[14px] w-full"
  ...
  return (
    <div className="border w-full h-[180px] rounded-[10px] text-center flex items-center justify-center">
      <div>
        ...
        {Array.isArray(info) ? (
          info.map((item, index) => (
            <Link key={index} to={`${linkType}${item}`} className="text-[18px] block">
              {item}
            </Link>
          ))
        ) : (
          <Link to={`${linkType}${info}`} className="text-[18px] max-w-[250px] inline-block">
            {info}
          </Link>
        )}
      </div>
