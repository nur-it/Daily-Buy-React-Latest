...
const AboutSection = ({ title, description, image, imagePosition }) => {
  return (
    <div className={`flex ${imagePosition === "left" ? "flex-col lg:flex-row" : "flex-col-reverse lg:flex-col"} items-center gap-[36px] mt-[56px]`}>
      <div>
        <h1 className="text-2xl lg:text-4xl text-[#383838] font-bold">{title}</h1>
        <p className="text-sm lg:text-lg text-[#626463] my-[32px]">{description}</p>
        <div className="w-full h-[300px] md:h-[450px]">
          <img className="w-full h-full object-center object-cover rounded-[10px]" src={image} alt="" />
        </div>
      </div>
    </div>
  );
    ...
    <>
      <div className="container mt-[80px] lg:mt-[52px] mb-[20px] lg:mb-[20px]">
        <div className="flex flex-col lg:flex-row items-center gap-[48px] ">
          ...
          <div>
            <h1 className="text-2xl lg:text-4xl text-[#383838] font-bold">A wonderful serenity has taken possession,</h1>
